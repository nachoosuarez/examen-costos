...
2. **Encofrado**: en losas, solo la cara **inferior** por fuera de las vigas (si un lateral de losa no tiene viga, ahí sí hay que encofrar el lateral). Vigas y losas están suspendidas → encofrado inferior salvo que la letra diga lo contrario. Zapatas/muros de contención → apoyan en el suelo, la cara inferior NO se encofra. **Criterio de "no restar" (Clase 4):** restar del encofrado áreas que en teoría no se encofran no siempre se justifica, cuando encofrar ese contorno recortado implica una dificultad/consumo de recursos mayor que encofrar liso — ej. en una losa en voladizo, no restar el área de las vigas de apoyo o de las paredes contra las que topa.
...
**¡Discrepancia importante entre la planilla oficial y el material de clase actual!** La Clase 4 del curso 2026 (`CLASES TEORICAS/Clase 4 (3).pdf`, diapositivas 125-128) usa este MISMO muro de Marzo-2020 como ejemplo, pero con criterios ligeramente distintos a los de la planilla oficial de arriba: (1) **encofrado como UNA SOLA CARA CONTINUA que incluye la pata**, `E=(0,2+2,4)×45=117 m²` (Tenor=4,6), en vez de la pata con 2 caras + muro con 1 cara =126 m² (Tenor=5,0) de la planilla oficial del examen real; (2) la barra "horquilla" con longitud unitaria **6,56 m** (desarrollo explícito de cada tramo + 2×10Ø de gancho) en vez de 6,28 m, dando un acero total de 2.239,2 kg y Cuantía 88,9 kg/m³ (vs. 2.177,28 kg y 86,4 kg/m³ de la planilla oficial). **Ambos son válidos** — la planilla oficial es la que calificó el examen real de 2020; la Clase 4 es el criterio con el que el equipo docente ACTUAL explica el mismo problema en 2026. Si el examen de este año no aclara el criterio de encofrado de la pata en un muro por trincheras, **usar el criterio de la Clase 4 (más reciente y explícitamente del curso vigente): una sola cara continua para todo el perfil (pata+pantalla), sin duplicar la pata**. Ver la nota completa en `Soluciones/202003 Ex Marzo/RESOLUCION.md`.
...
**Decodificación de cotas "acumuladas"/"corridas" en un plano (visto en Ene-2026):** algunos planos NO acotan cada tramo por separado, sino que ponen, junto a cada línea de referencia (vertical u horizontal), un único número que es su **posición absoluta en un eje de coordenadas del proyecto** (parecido a un replanteo topográfico, con decimales tipo "4065,9"), no la distancia al elemento vecino. Para sacar CUALQUIER medida hay que **restar dos cotas de la misma familia de líneas** (nunca usar un solo número aislado como si fuera una distancia). Señal de que se está leyendo bien: si la Δ entre dos cotas cercanas coincide EXACTO con una dimensión ya conocida por otro dato (ej. la sección de un pilar o viga dada en su rótulo, "P51 60x20" → buscar un par de cotas con Δ=60,0), y si el **vano libre final da un número redondo** (aunque las cotas originales tengan decimales sueltos) — ambas son fuertes señales de una lectura correcta; si no da redondo, revisar qué par de cotas se está restando (a veces hay 2 pares superpuestos en la misma línea: uno para el ancho del PILAR y otro, distinto, para el ancho de la VIGA que llega hasta ese pilar). Vano libre entre dos elementos = cota de la cara de un elemento (la más cercana al vano) − cota de la cara del otro elemento (la más cercana al vano, del otro lado). Ver desarrollo completo con la tabla de verificación cruzada en `Soluciones/202601 Examen Enero 2026/RESOLUCION.md`.
...
**Mini-ejemplo (Dic-2025):** relación 3,5:2:1, a/c=0,50, dens. cemento (aparente) 1.400 kg/m³. Volúmenes reales (aparente×coef.aporte): grueso 3,5×0,55=1,93; fino 2×0,57=1,14; cemento 1×0,47=0,47; agua (kg=a/c×kg cemento aparente=0,50×1.400=700kg → 0,70 m³, coef=1) → Σreales=4,24 m³ (volumen de hormigón que rinde el batch completo). Para pasar a 1 m³ real, escalar cada aparente por 1/Σreales=1/4,24 (atajo algebraico de arriba): cemento=1,00/4,24=0,236 m³ → ×1.400=330,58 kg = 13,22 bolsas × 220 $/bolsa = 2.909 $; grueso=3,50/4,24=0,83 m³ × 1.250 $/m³ = 1.033 $; fino=2,00/4,24=0,47 m³ × 850 $/m³ = 401 $. Total materiales ≈ **4.344 $/m³**. (⚠️ Ojo: NO se calcula multiplicando el volumen *real* del cemento por la densidad *aparente* — 0,47×1.400 no es un paso válido de esta cuenta; hay que dividir el aparente por Σreales, no multiplicar el real por la densidad aparente.)
...
Comitente extiende a 18 meses; CF mensual nuevo=90% del CF mensual promedio original (30/12×0,90)
→ CF nuevo total=90%×(30/12)×18=40,50. Costo nuevo=110,50. **Sin reconocimiento:** Beneficio
...
**Atajo para descartar una línea de entrada (visto en Marzo-2020):** si el déficit máximo **sin intereses** ya supera por sí solo el límite de la línea más chica (ej. déficit $3.000 vs. línea 1 de límite $2.000), ni hace falta calcular intereses para descartarla — directamente no alcanza, pasar a la línea siguiente. Recién ahí, sobre la línea elegida, sumar el costo financiero mes a mes (interés = tasa mensual × déficit acumulado del mes anterior, si es negativo) para obtener el monto máximo real a usar y el total de intereses. Ejemplo: déficit sin interés $3.000 (mil $, mes 4) > límite Línea 1 ($2.000) → se descarta sin más cálculo; con Línea 2 (límite $4.000, 3%/mes) el déficit con intereses compuestos sube a $3.030 y el total de intereses da $120. Ver desarrollo completo en `Soluciones/202003 Ex Marzo/`.
...
**Ejercicio tipo "Estado Económico-Financiero completo de una obra" (Clase 20, aparece como pregunta conceptual con desarrollo numérico, ej. Feb-2025):** distinto de comparar alternativas de inversión (arriba) — acá se arma el flujo de UNA sola obra para calcular su Costo Financiero real. Paso a paso (con los números del ejemplo de la Clase 20, Costo Total=$1.000.000, Beneficio=15% s/venta → Venta=1.000.000/(1−0,15)=$1.176.471):
1. **Estado Económico** (rentabilidad, sin importar el momento del cobro/pago) = `Venta−Costo=Utilidad`. Desglosar el Costo Total en insumos (Mano de Obra, Equipos, Materiales, Suministros y Subcontratos, Varios) con su % de incidencia, y separar Costos Indirectos (van dentro del Costo Total) del Beneficio (% sobre la Venta, no sobre el Costo — pasaje Costo→Venta = 1/(1−%beneficio), en este caso ×1,3219).
2. **Cronograma físico de avance** de cada rubro/insumo mes a mes (suma 100% por fila) → de ahí, **Previsión de Costos por mes** (costo total del insumo × su % de avance ese mes) y **Previsión de Ventas Mensuales** (venta total del rubro × % de avance) → **Estado Económico Mensual** = Venta del mes−Costo del mes = Utilidad del mes (puede dar negativa en meses tempranos, se recupera en los siguientes; el acumulado converge al beneficio total, $176.471=15%).
3. **Estado Financiero** (disponibilidad de caja) = `Ingreso−Egreso=Disponibilidad`. Cada insumo tiene su propio desfasaje de pago (ej. Mano de Obra 50% en el mes + 50% al mes siguiente; Equipos/Materiales a 30 días; Suministros a 60 días) y los ingresos por venta también el suyo (ej. a 60 días de certificado). Armar el **Flujo de Fondos Mensual sin Costo Financiero** = Ingresos del mes−Egresos del mes, y su acumulado — si da negativo en algún mes, ahí la obra necesita financiación externa.
4. **Costo Financiero** = interés mensual `i=(1+I_anual)^(1/12)−1` aplicado sobre cada saldo acumulado negativo del paso 3, mes a mes. Se resta del flujo de fondos → el **Beneficio real baja** del "de catálogo" (en el ejemplo, de 15,00% a 14,46%, Costo Financiero=0,54% de la venta).
5. **Atajo equivalente por Valor Actualizado** (mismo resultado que el paso 4, sin armar el flujo mes a mes con intereses): `Margen real = VNA(Cobros)−VNA(Pagos) / VNA(Cobros)`, actualizando cada mes con el coeficiente `1/(1+i)^n`. Da el mismo 14,26%≈14,46% (pequeña diferencia de redondeo entre los 2 métodos en el propio ejemplo de la Clase 20).
6. **Un anticipo del cliente reduce fuertemente la necesidad de financiación** — en el mismo ejemplo, con un anticipo del 20% al inicio (Mes 0) el Costo Financiero cae de 0,54% a **0,03%** de la venta (de $6.333 a $300), acercando la obra a la "autofinanciación". Ver también la Guía sección 7 para el costo de la garantía que hay que constituir a cambio de recibir ese anticipo.
...
- Ene-2026: Metraje de losas con planilla de "cotas acumuladas" DECODIFICADA (tipo NUEVO: coordenadas de replanteo en vez de cotas de tramo) + planilla en "cruz" (confirma que puede sobrar un número sin explicar, ya visto en Feb-2024), metraje/consumos resueltos con espesor de losa ilustrativo (dato no incluido en el plano ni el enunciado), costo de equipos (retroexcavadora), teoría de mano de obra/licitaciones/garantías.
...
**Pendiente para próximas corridas:** la planilla de armado en "cruz" ya tiene su regla principal decodificada (diámetro arriba+paso abajo=una familia; diámetro izquierda+paso encerrado en la caja=la otra familia — confirmado contra solución oficial en Feb-2024/Jul-2022 y reconfirmado en Ene-2026), con la salvedad recurrente de que puede sobrar un número sin explicación (se descarta). Sigue pendiente conseguir el **espesor real de la losa de Ene-2026** (no estaba en el plano ni el enunciado, se usó un valor ilustrativo) para cerrar del todo esa Parte I/II. También quedan variantes de metraje (tanques, escaleras, zapatas aisladas) a medida que aparezcan en exámenes más viejos.
